...
fn main() -> anyhow::Result<()> {
    ...
    let mut add_tests = |name: &str, fds_init_helper: fn() -> (libc::c_int, libc::c_int)| {
        // add details to the test names to avoid duplicates
        let append_args = |s| format!("{s} <fds={name:?}>");
    ...
    add_tests("udp", udp_fds_init_helper);
    add_tests("unix-dgram", unix_dgram_fds_init_helper);
