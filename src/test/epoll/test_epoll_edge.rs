...
fn test_not_writable_when_write(readfd: libc::c_int, writefd: libc::c_int) -> anyhow::Result<()> {
    let epollfd = epoll::epoll_create()?;
...
fn pipe_fds_init_helper() -> (libc::c_int, libc::c_int) {
    let (readfd, writefd) = unistd::pipe().unwrap();
    (readfd, writefd)
}

struct EpollEdgeTests {
    filter_shadow_passing: bool,
    filter_libc_passing: bool,
    summarize: bool,
    tests: Vec<test_utils::ShadowTest<(), anyhow::Error>>,
}
...
impl EpollEdgeTests {
    ...
    pub fn add_stream_tests(
        ...
        self.tests.extend(vec![ShadowTest::new(
            &append_args("not-writable-when-write"),
            move || {
                let (readfd, writefd) = fds_init_helper();
                test_not_writable_when_write(readfd, writefd)
            },
            all_envs.clone(),
        )]);
    }

    pub fn add_datagram_tests(
        &mut self,
        fds_name: &str,
        fds_init_helper: fn() -> (libc::c_int, libc::c_int),
    ) {
    ...
    pub fn run_tests(&mut self) -> anyhow::Result<()> {
        if self.filter_shadow_passing {
            self.tests.retain(|x| x.passing(TestEnvironment::Shadow));
        }
        if self.filter_libc_passing {
            self.tests.retain(|x| x.passing(TestEnvironment::Libc));
        }

        test_utils::run_tests(&self.tests, self.summarize)?;
...
fn main() -> anyhow::Result<()> {
    ...
    let mut tests = EpollEdgeTests {
        filter_shadow_passing,
        filter_libc_passing,
        summarize,
        tests: vec![],
    };

    tests.add_stream_tests("tcp", tcp_fds_init_helper);
    tests.add_datagram_tests("udp", udp_fds_init_helper);
    // add_common_tests("pipe", pipe_fds_init_helper);
