...
impl InitialState {
    ...
    fn recvmsg(
        ...
        if ifaddrmsg.ifa_prefixlen != 0 || ifaddrmsg.ifa_flags != IfaFFlags::empty()
            ...
            log::warn!("Unsupported ifa_prefixlen, ifs_flags, ifa_scope, or ifs_index (they have to be 0)");
            writer.write(&errmsg_buffer)?;
            return Ok(errmsg_ret);
        }
        ...
        for interface in interfaces {
            ...
            let attrs = [
                ...
                Rtattr::new(None, Ifa::Label, Buffer::from(Vec::<u8>::from(interface.label))).unwrap(),
            ];
            let ifaddrmsg = Ifaddrmsg {
                ...
                ifa_index: interface.index.try_into().unwrap(),
        ...
        let done_msg = {
            ...
            let seq = Some(nlmsg.nl_seq);
            let pid = None;
            let payload: NlPayload<Nlmsg, Ifaddrmsg> = NlPayload::Empty;
            Nlmsghdr::new(len, nl_type, flags, seq, pid, payload)
        };
